...
public class DecoraciónPanel extends JPanel {
    public void paintComponent(Graphics g) {
        ...
        int[] xPoints = {0, width / 3, width / 3 + d, 0 + d}; // Coordenadas "x" de los puntos del polígono
        ...
        g2d.setColor(colorlínea);
        g2d.drawLine(width / 3 + d, 0, 0 + d, height); // Ajustar el punto de inicio en el eje "y" a 0
        ...
        g2d.drawLine(width / 3 + d + d2, 0, 0 + d + d2, height); // Primera línea fina
        g2d.drawLine(width / 3 + d + 2 * d2, 0, 0 + d + 2 * d2, height); // Segunda línea fina

        // Rellenar el espacio entre las nuevas líneas
        int[] xPoints2 = {width / 3 + d + d2, width / 3 + d + 2 * d2, 0 + d + 2 * d2, 0 + d + d2}; // Coordenadas "x" de los puntos del nuevo polígono
